...
function Mark({mark,index,light}:{mark:ChalkMark;index:number;light:boolean}){
 ...
 if(mark.type==='text'){const available=Math.max(30,430-mark.x),estimated=mark.text.length*mark.size*.53;return <text className="chalk-word" x={mark.x} y={mark.y} fontSize={mark.size} textLength={estimated>available?available:undefined} lengthAdjust="spacingAndGlyphs" fill="currentColor" stroke="none" style={style}>{mark.text}</text>;}
...
export default function TeachingBoard({sketch,onClose,light=true,onToggle,interactive=false,busy=false,onAnswer,onFeedback}:{sketch:BoardSketch;onClose:()=>void;light?:boolean;onToggle?:()=>void;interactive?:boolean;busy?:boolean;onAnswer?:(question:string)=>void;onFeedback?:(text:string)=>void}){
 const [chosen,setChosen]=useState<number|null>(null);
 const challenge=sketch.challenge;
 return <aside className={`teaching-board ${light?"whiteboard":""}`} aria-label={light?"Miss Lumi’s whiteboard":"Miss Lumi’s chalkboard"}>

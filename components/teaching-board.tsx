...
function Mark({mark,index,light}:{mark:ChalkMark;index:number;light:boolean}){
 const style={color:light?({chalk:"#28413c",sage:"#367d62",gold:"#95742c"}[mark.tone]):colors[mark.tone],animationDelay:`${Math.min(index*.09,1.7)}s`};
...
export default function TeachingBoard({sketch,onClose,light=false,onToggle}:{sketch:BoardSketch;onClose:()=>void;light?:boolean;onToggle?:()=>void}){

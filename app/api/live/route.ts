...
export async function POST(r:Request){try{
 ...
 const contract=d.classroom?teacherContract:d.topic==='shadows'?shadowContract:scienceContract;
 ...
 const upstream=await openai('live/sessions',{session:{model:'gpt-live-1',instructions:contract+` Speak warmly to a child aged ${d.age==='5–7'?'5 to 7':'8 to 10'}. One short sentence at a time; listen to interruptions. Current challenge: ${challenge} ${controls} Delegate scientific reasoning and hints to the client. Never answer predictions or unlock discoveries. Do not claim you see the camera unless vision observations were supplied.`,delegation:{type:'client'}},transport:{type:'webrtc',sdp:d.sdp}},25000);

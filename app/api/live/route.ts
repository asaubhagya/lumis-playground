...
export async function POST(r:Request){try{
 ...
 const upstream=await openai('live/sessions',{session:{model:LIVE_MODEL,audio:{output:{voice:'marin'}},instructions:contract+` Speak warmly to a child aged 8 to 12. One short sentence at a time; listen to interruptions. Current challenge: ${challenge} ${controls} Delegate only reasoning or control changes that require backend help. Never answer predictions or unlock discoveries. Do not claim you see the camera unless vision observations were supplied.`,delegation:{type:'client'}},transport:{type:'webrtc',sdp:d.sdp}},25000);

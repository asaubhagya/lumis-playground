...
export function validateBoard(value:unknown,phase:unknown):BoardSketch|null{
 ...
 // Keep generated labels legible when their estimated text bounds collide.
 const labels:Extract<ChalkMark,{type:'text'}>[]=[];
 for(const mark of elements){if(mark.type!=='text')continue;
  const width=Math.min(430-mark.x,mark.text.length*mark.size*.53);
  const candidates=[mark.y,...Array.from({length:9},(_,i)=>[mark.y-(i+1)*26,mark.y+(i+1)*26]).flat()].filter(y=>y>=mark.size&&y<=248);
  const free=candidates.find(y=>!labels.some(other=>Math.abs(other.y-y)<Math.max(other.size,mark.size)+4&&mark.x<other.x+Math.min(430-other.x,other.text.length*other.size*.53)+8&&mark.x+width+8>other.x));
...
Use strokes for freehand outlines, curves, objects, diagrams, number lines, equations, patterns, or simple pictures. Use a closed stroke to draw a rectangle or polygon. Arrows end at the final point. Do not use SVG, HTML, code, image URLs, or unsupported primitives. Keep it legible: usually 6–16 elements, max 24, few labels of 1–3 words, at least 25px apart, and keep words away from the right edge. Use mainly chalk and sage. For concepts that need colors, such as rainbows, you may use tones rose (red), orange, gold (yellow), sage (green), sky (blue), and violet. Use these meaningfully; a rainbow diagram needs distinct colored bands, not a single monochrome arch. Labels must match the geometry. Physically ground the drawing: light travels straight and opaque objects block it. Draw what helps the question, not a fixed template. The board can illustrate any foundational idea needed for the explanation; welcome other age-appropriate subjects, including numbers, nature and everyday science. Explain the requested idea fully enough to be useful, then offer to return to the experiment; do not force an unrelated drawing into the current topic. Include a board when the learner asks to draw, write, sketch, or paint, and when a visual clearly helps. Use the board like a patient teacher, asking one small question. During any prediction board must be null: do not reveal the answer, tell the child to recall observations and choose, and do not suggest moving paused controls. Never claim to draw until the application reports it. Keep all explanations appropriate for ages 8–12.`;

export const openBoardContract=` In open classroom mode, start with one warm curiosity prompt, not a menu of topics. Follow any age-appropriate question, including a change of subject. Every substantive explanation needs a fresh, clear sketch: one large recognizable picture occupying most of x:30..405 and y:35..215, meaningful arrows, and 2–4 short handwritten labels placed next to what they describe. Use the note for a simple takeaway sentence, not another heading. Check the science and diagram before responding: arrows must show the correct direction, labels must correspond to actual objects, and drawings must show the mechanism. For Moon phases the Sun always lights half the Moon; we see different amounts of that sunlit half as the Moon orbits Earth, not an Earth shadow or a changing amount of illumination. Avoid a board of words alone. Keep labels apart and leave generous empty space; split complex ideas over follow-up drawings instead of cramming. In the spoken text explain what to notice in the drawing, in 2–4 short concrete sentences, ending with one specific follow-up question. A multiple-choice challenge is optional: use it when the child is ready to try, not on every question or explanation. When useful, include: {"question":"one simple question about this picture","options":["short choice","short choice"],"correct":0,"explanation":"why, in one kind concrete sentence"}. Use 2 or 3 choices and a zero-based correct index. This is a small check of understanding, not a test or proof of mastery. Ensure one unambiguously correct choice. Do not reveal the correct choice in the question. The application reveals the explanation after the child selects. Keep the diagram accurate and visually self-contained. For a new concept, prefer an inviting concrete picture over text or equations. On a follow-up, progress from the child's response rather than repeating the same question. Never add a challenge during a preset prediction. There is no student ink or camera. Invite a spoken or typed response.`;

export const visualTeachingContract=` Your whiteboard is the primary explanation, not a decorative doodle. Plan the layout before drawing. Use most of the 440x260 space: two or three meaningful groups with clear spacing. Text labels must not overlap shapes, arrows, or each other. Reserve at least text.length × fontSize × 0.6 pixels horizontally for each label; never put a word in a narrow box. Do not draw a long summary sentence inside the canvas: use board.note for that. Prefer 16–20px handwritten labels, 1–3 words each, at least 30px vertical separation, and no more than 25 total marks. Use arrows or side-by-side examples to show the causal mechanism. Put the key takeaway in board.note. For AI show examples -> training -> a learned pattern -> a new example; distinguish learning statistical patterns from human understanding. For history a short ordered timeline and one causal link can help. For seasons show Earth's tilted axis, sunlight angle and opposite hemispheres; distance from Sun is not the main cause. For space label any simplified layout as not to scale. Choose a diagram that answers the actual question, not a generic picture. Explanations remain on the board while a separate illustration is generated. End the spoken explanation with one thought-provoking but answerable question.`;
